...
#---- now  RRF2 algo
	...
	for(z in 1:length(mf_cols)) {
		...
		if(!is.na(tstat)) if(as.numeric(tstat)) thesevars[(nrow(thesevars)+1),] <- cbind(mf_cols[z],tstat)  #----- here we would have the tstat>2 rule like this
						# if(as.numeric(tstat)>2) thesevars[(nrow(thesevars)+1),] <- cbind(mf_cols[z],tstat)
			}
