#Public Sector, Total assets
#-----	PSBSFAD	PS|XDCG6TW0|S1
...
cat(x,this_country,iso2,iso3,imf_countrycode,this_indicator,"\n",sep="_")
		...
		fname <- paste(this_dir,'IMF_PUB_ASSETS_',iso3,'.txt',sep="")
		queryfilter <- list('CL_FREQ'='A',"CL_PSBS Country_PSBSFAD"=imf_countrycode, "CL_FAD Input Indicators_PSBSFAD"=this_indicator)
			result <- try(
				suppressMessages(PUB_ASSETS <- CompactDataMethod(databaseID,queryfilter,startdate,enddate,FALSE,FALSE,FALSE))
			)
			if(class(result) == "try-error" || is.null(result)) {
				cat("NO ",fname,":     x=",x,"_",this_country,"_",iso2,"_",iso3,"\n",sep="")
			} else if(!is.null(PUB_ASSETS)) {
				res <- data.frame(PUB_ASSETS$Obs[1])
				names(res) <- c('year_mo','PUB_ASSETS')
