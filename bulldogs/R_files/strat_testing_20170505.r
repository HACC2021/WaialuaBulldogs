...
############################
2017-05-05 lets do some picking
...
plot_finalset <- function (finalset,outname) {	
		...
		y <- finalset$idx_ret
		par(pch=22, col="black") # plotting symbol and color 
		heading = paste("S&P 1500 performance quantiled ") 
		plot(x, y, type="n",lty="solid",lwd=0,main=heading,
					xaxp		=	c(min(finalset$idx),max(finalset$idx),10),
					yaxp		=	c((min(vv)-.10),max(vv),6),
					xlim		=	c(min(x),max(x)),
		...
		legend(min(x),max(vv),
			c('Lowest','2ndLowest','Highest','2ndHighest','SP 1500'),
#				names(finalset)[c(2:12],
			lty=c(rep(1,3)),
#				lty=c(rep(1,11)),
			lwd=c(3,
#					2,2,2,2,2,2,2,2,
				3,3),
			col= c(colors()[258],'green3',
#					colors()[132],colors()[129],colors()[125],
#					colors()[142],colors()[146],colors()[150],
...
strats <- c('tl_ta','ni_ta','mkt_leverage','nimta','cashmta','mtb','nimta_m1q')
for(strat in strats){
datez <- tq("select left(datadate,6), max(datadate) from therocrates.dbo.hzrd_3morets_test group by left(datadate,6) order by left(datadate,6)")[,2]
thesedates <- datez[3]
for(z in 4:length(datez)) {
		if(z%%3==0) thesedates <- append(thesedates,datez[(z)])
}
...
finalset <- data.frame(matrix(NA,nrow=length(thesedates),ncol=12))
	...
	for(y in 1:length(thesedates)) {
		set <- tq(paste("select datadate, gvkey, ",strat,", moret_3 as ret from therocrates.dbo.hzrd_3morets_test where datadate='",
				thesedates[y],"' and moret_3 is not null", sep=""))

			for(x in 1:ncol(set)) set[,x] <- as.character(set[,x])
				set[,strat] <- as.numeric(set[,strat])
				set$ret <- as.numeric(set$ret)
		set <- set[order(set[,strat],decreasing=F),]
			n <- trunc(nrow(set)/10)
		ret_set1 <- mean(set[1:n,'ret'])
		ret_set2 <- mean(set[(n+1):(n*2),'ret'])
		ret_set3 <- mean(set[(n*2+1):(n*3),'ret'])
	...
	plot_finalset(finalset,paste("c:/therocrates/",strat,"_top_bottom_1995",sep=""))

	#----- since 2000
	finalset <- finalset_rets[finalset_rets$data_date>'19991231',]
	thesedates  <- thesedates[thesedates>'19991231']
	...
	plot_finalset(finalset,paste("c:/therocrates/",strat,"_top_bottom_2000",sep=""))

	#----- since 2006
	finalset <- finalset_rets[finalset_rets$data_date>'20050930',]
	thesedates  <- thesedates[thesedates>'20050930']
	...
	plot_finalset(finalset,paste("c:/therocrates/",strat,"_top_bottom_2006",sep=""))
	...
	plot_finalset(finalset,paste("c:/therocrates/",strat,"_top_bottom_2012",sep=""))
	#----- since 2014
	finalset <- finalset_rets[finalset_rets$data_date>'20130930',]
	thesedates  <- thesedates[thesedates>'20130930']
	...
	plot_finalset(finalset,paste("c:/therocrates/",strat,"_top_bottom_2014",sep=""))
