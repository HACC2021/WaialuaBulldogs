...
runthese <<- function(trythese,wf) {
	...
	base_formula <- c('MVIXM','MCREIDX_R1Y_NON','MHOUSEIDX_R1Y_NON','MDJIDX_R1Y_NON','MCPI','MBBBSPD','MTSLOPE','MUNEMP')
	large_co_facs <- c('MCPIASIA','MCPIEUR','MCPIJPN','MCPIUK','MFXASIA','MFXEUR','MFXJPN','MFXUK','MRGDPASIA','MRGDPEUR','MRGDPJPN','MRGDPUK')
	base_plus <- list(c('MRGDP','MNGDP'), c('MUNEMP_D1Y_NON','MUNEMP_D6M_NON','MUNEMP_D2Y_NON'),c('MPRIME','MTREAS3MO','MTREAS5YR','MTREAS10Y','MMORT'),
							c('MPRIME_D1Y_NON','MTREAS3MO_D1Y_NON','MTREAS5YR_D1Y_NON','MTREAS10Y_D1Y_NON','MMORT_D1Y_NON'), c('MRDISINC','MNDISINC'))

	gids <- nq(paste("select top 1 g.market_cap, k.gsubind, k.class, k.country as loc, conm from MFSQ.dbo.KRIS_COMPANY_TICKER k
		left join MFSQ.dbo.CCAR_LARGE_CAPS g on k.gvkey=g.gvkey where k.gvkey='",gvkey,"'",sep=""))

	if(nrow(gids)==0) {
		gids <- nq(paste("select 1000 as market_cap, s.gic as gsubind, c.class, 'US' as loc, gic.gicdesc as conm 
