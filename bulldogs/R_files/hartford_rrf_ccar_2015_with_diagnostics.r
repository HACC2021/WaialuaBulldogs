...
#-----THE WHEEL OF FORTUNE
		...
		TRYS=10
			...
			if(length(wf)>0) {
				lset <- summary(lm(as.formula(paste(lside," ~ ",paste(wf,sep="",collapse=" + "),sep="")),testset))
				estimated.model <- lm(as.formula(paste(lside," ~ ",paste(wf,sep="",collapse=" + "),sep="")),testset)
				global.assessment <- summary(gvlma(estimated.model))			#	Global Model Assessment
				mc.vif <- vif(estimated.model)									#	Multicollinearity
				# Residual Analysis
...
#				qqPlot(estimated.model, main="QQ Plot")							# qq plot for studentized resid
