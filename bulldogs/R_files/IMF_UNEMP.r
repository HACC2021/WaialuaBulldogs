...
cat(x,"_",this_country,"_",iso2,"_",iso3,"\n",sep="")
		...
		fname <- paste(this_dir,'IMF_UNEMP_M_',iso3,'.txt',sep="")
		queryfilter <- list(CL_FREA="M", CL_AREA_IFS=iso2, CL_INDICATOR_IFS =c(this_indicator))

			result <- try(
				suppressMessages(UNEMP <- CompactDataMethod(databaseID,queryfilter,startdate,enddate,FALSE,FALSE,FALSE))
			)
			if(class(result) == "try-error" || is.null(result)) {
				cat("NO ",fname,":     x=",x,"_",this_country,"_",iso2,"_",iso3,"\n",sep="")
			} else if(!is.null(UNEMP)) {
				res <- data.frame(UNEMP$Obs[1])
				names(res) <- c('year_mo','UNEMP')
				write.table(res,fname,append=F,quote=F,sep='|',na='',row.names=F,col.names=F,fileEncoding='UTF-8')
			}
			
			file_check <- dir(this_dir,paste('IMF_UNEMP_M_',iso3,'.txt',sep=""))
			if(length(file_check)==0){

				fname <- paste(this_dir,'IMF_UNEMP_Q_',iso3,'.txt',sep="")
