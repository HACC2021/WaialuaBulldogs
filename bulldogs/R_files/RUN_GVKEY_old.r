run_gvkey <- function (gvkey='006066',colz=names(mfacs)[-1]) {
	...
	#----- first we find our starting factor and order model inputs by tscores descending
				...
				cat("\tError in number of rows less then number of factors to be tested\n")
				return()
				}
				...
				model_ids <- sub('algo_id_','',names(winning_formulas)[-1])
		...
		#----- diagnostics
					...
					for(x in 1:length(model_inputs)) sf_resid_normality[x,2] <- sfTest(testset[,model_inputs[x]])@test$statistic
					
		#----- upper and lower bounds
					bounds <- data.frame(matrix(nrow=length(model_inputs),ncol=3))
						names(bounds) <- c('rf_code','ub','lb')
					for(x in 1:length(model_inputs)){
							conf_intervals	<- CI(testset[,model_inputs[x]][!is.na(testset[,model_inputs[x]])],ci=0.95)
		...
		#-----	we have formulas / estimated models, now lets run diagnostics
			...
			#				GA 		<- summary(gvlma(EM)) 			#	Global Model Assessment							NOT USING THIS ONE
			#				GQ 		<- gqtest(EM)					#	Goldfeld-Quandt test for homoskedasticity		NOT USING THIS ONE AND IT BREAKS SOMETIMES
			#				OT 		<- outlierTest(EM)				#	Bonferonni p-value for most extreme obs			NOT USING THIS ONE
			#				SW <- shapiro.test(residuals(EM))		#	Shapiro-Wilk test for normality	or residuals	NOT USING THIS ONE
			#				SF <- sfTest(residuals(EM))				#	Shapiro-Francia test for normality				NOT USING THIS ONE
					...
					reg_master_set <- data.frame(matrix(nrow=1,ncol=22))
							names(reg_master_set) <- c('reg_id','reg_name','reg_p_value','adj_r_sqr','error','aic','bic','vif_mean','bp_hetero_lm',
											'dh_sercorr_lag1','dh_sercorr_lag2','dh_sercorr_lag3','dh_sercorr_lag4',
											'bg_sercorr_lag1','bg_sercorr_lag2','bg_sercorr_lag3','bg_sercorr_lag4',
											'num_obs','num_dof','dof','f_value','r_sqr')
							...
							reg_master_set$bg_sercorr_lag1		<-	round(bgtest(EM, order=1)$p.value,6)			
							reg_master_set$bg_sercorr_lag2		<-	round(bgtest(EM, order=2)$p.value,6)
							reg_master_set$bg_sercorr_lag3		<-	try(bgtest(EM, order=3)$p.value,6)
								reg_master_set$bg_sercorr_lag3		<- ifelse(is(reg_master_set$bg_sercorr_lag3,'try-error'),NA,round(reg_master_set$bg_sercorr_lag3,6))
							reg_master_set$bg_sercorr_lag4		<-	try(bgtest(EM, order=4)$p.value,6)
								reg_master_set$bg_sercorr_lag4		<- ifelse(is(reg_master_set$bg_sercorr_lag4,'try-error'),NA,round(reg_master_set$bg_sercorr_lag4,6))
